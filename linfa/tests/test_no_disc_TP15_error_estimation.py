...
def run_test():
    ...
    def log_density(calib_inputs, model, surrogate, transform):
        ...
        # Initialize negative log likelihood (batch size x 1)
        ...
        for loopA in range(num_reapeat_obs):
            ...
            # - n / 2 * log (sigma)
            l2 = -0.5 * model.data.shape[0] * torch.log(std_dev**2)
            
            # - 1 / (2 * sigma^2) sum_{i = 1} ^ N (eta_i + disc_i - y_i)^2 
            l3 = -0.5 / (std_dev ** 2) * torch.sum((modelOut - Data[:,loopA].unsqueeze(0))**2, dim = 1)
    ...
    def log_prior(calib_inputs, transform):
        ...
        if False:
            xvalues = torch.linspace(0, 0.2, 100)
            pdf = np.exp(std_dev_ratio.log_prob(xvalues))
            plt.plot(xvalues.detach().numpy(), pdf.detach().numpy())
            plt.show()
            exit()
