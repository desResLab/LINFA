...
def run_test():
    ...
    exp.calibrate_interval  = 2000           #:int:    How often the surrogate model is updated
        ...
        for loopA in range(num_obs):
            ...
            if(False):
                ...
                for loopB in range(discrepancy.size(0)):
                    print('%15.3f %15.3f %15.3f %15.3f' % (torch.mean(modelOut[:,loopB],dim=0),discrepancy[loopB,0],modelOut[0,loopB]+discrepancy[loopB,0],Data[loopB,loopA]))
                print('')
