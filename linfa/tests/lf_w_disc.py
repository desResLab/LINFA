...
def run_test():
    ...
    exp.run_nofas           = False         # TODO turn this back on eventually
    ...
    exp.transform = trsf # TODO figure out whats happening when use_surrogate = False
    ...
    # TODO: generalize this to take on multiple temperatures and pressures
    # var_grid_in = torch.tensor(exp.model.data[:,:2])
    # var_grid_out = torch.tensor(exp.model.data[:,2:])
    var_grid_in = torch.tensor(exp.model.data[0:2])
    var_grid_out = torch.tensor(exp.model.data[2:])
    ...
    def log_density(calib_inputs, model, surrogate, transform):
        ...
        if (surrogate is None):
            discrepancy = torch.zeros(modelOut.shape).t() #HAD TO SPECIFY SHAPE
        else:
            # (num_var)
        ...
        # TODO: generalize to multiple experiments
        ...
        for loopA in range(num_obs):
            ...
            l2 = (-0.5 * langmuir_model.data.shape[0] * torch.log(torch.prod(stds))).item()
            l3 = -0.5 * torch.sum(((modelOut + discrepancy.t() - Data[loopA].unsqueeze(0)) / stds.t())**2, dim = 1) 
