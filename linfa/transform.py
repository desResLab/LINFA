    ...
    def __init__(self, func_info=None):
        ...
        for func, a, b, c, d in func_info:
            if func == "identity":
                ...
                m2 = (c + d) / 2
                t2 = (d - c) / 2
                self.funcs.append(lambda x: 
                                  torch.tanh((x - m1) / (b - a) * 6.0) * t2 + m2)
                self.log_jacob.append(lambda x: 
                                      torch.log(1.0 - torch.tanh((x - m1) / (b - a) * 6.0) ** 2) 
                                      + np.log(t2) + np.log(3.0) - np.log(t1))
            elif func == "linear":
                self.funcs.append(lambda x: (x - a) / (b - a) * (d - c) + c)
                self.log_jacob.append(lambda x: np.log(d - c) - np.log(b - a))
            elif func == "exp":
                self.funcs.append(lambda x: 
                                  torch.exp((x - a) / (b - a) * (np.log(d) - np.log(c)) + np.log(c)))
                self.log_jacob.append(lambda x: 
                                      (x - a) / (b - a) * (np.log(d) - np.log(c)) + np.log(c) + 
